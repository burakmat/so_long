...
int is_player_dead(t_game *game)
{
	...
	{
		if (game->foe[i].row == game->player.row && game->player.pos_x <= game->foe[i].pos_x + 60 && game->player.pos_x >= game->foe[i].pos_x - 150)
		{
			game->foe[i].state = 0;
...
int is_target_active_collectible(t_game *game, int row, int column)
{
	...
	{
		if (game->map.collectible[i].row == row && game->map.collectible[i].column == column && game->map.collectible[i].active)
		{
			return (1);
		}
		++i;
